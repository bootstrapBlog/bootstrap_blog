...
module.exports = function(cb){
  ...
  async.series([
    ...
    function(callback){
      rl.question('What is the url of your database? (localhost:27017/bootstrap_blog) ', function(url){
    ...
    function(callback){
      rl.question('Admin E-Mail for your blog: ', function(email){
        config.email = email;
        callback(null);
      });
    },
    function(callback){
      ...
      database.checkIfUserExistIfNotCreate(config.username, config.password, config.email);
      config.password = '******';
      console.log(config);
